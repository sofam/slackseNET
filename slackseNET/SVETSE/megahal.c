...
/* 
 * Mods by sjadoh:
 * 		- stripped macintosch / dos support
 * 		- changed io letting terminal handle allinging 
 * 		- added learn mode feature
 *
 *	Main purpose of these mods where to make it better ineteract with
 * 	external programs.
 * 		
 */
...
COMMAND command[] = {
	...
	{ { 5, "LEARN" }, "enables learn mode, no reply on next message", LEARN },
...
int main(int argc, char **argv)
{
	...
	/* we want her too reply by default */
	replymode=TRUE;
	...
	fprintf(stdout, "\nMegaHAL by Jason Hutchens, mods by sjadoh.\n\n");
...
bool print_header(FILE *file)
{
	...
	fprintf(file, "Copyright (C) 1998 Jason Hutchens - MegaHAL 8.6 *mod*\n");
...
void write_output(char *output)
{
   ...
   char *bit;
	...
	if(bit==NULL) (void)status("Xenia: %s\n", formatted);
	while(bit!=NULL) {
		(void)status("Xenia: %s\n", bit);
		bit=strtok(NULL, "\n");
	}
